...
class CheckAnswersHelper @Inject()(dateOfArrivalSummary: WhenReceiveShortageExcessSummary,
                                   howGiveInformationSummary: HowGiveInformationSummary,
                                   giveInformationMovementSummary: GiveInformationMovementSummary) {
  ...
  def summaryList()(implicit request: DataRequest[_], messages: Messages): SummaryList =
    SummaryListViewModel(Seq(
      ...
      giveInformationMovementSummary.row()
    ).flatten)
}
