...
trait ReferenceDataHttpParser extends BaseConnectorUtils[CnCodeInformationResponse] {
  ...
  def post(url: URL, body: CnCodeInformationRequest)(implicit hc: HeaderCarrier, ec: ExecutionContext, writes: Writes[CnCodeInformationRequest]):
  Future[Either[ErrorResponse, CnCodeInformationResponse]] = {
    http
      .post(url)
      .withBody(Json.toJson(body))
      .execute[Either[ErrorResponse, CnCodeInformationResponse]]
  }
}
