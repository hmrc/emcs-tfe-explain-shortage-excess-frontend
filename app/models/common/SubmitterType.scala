...
object SubmitterType extends Enumerable.Implicits {
  ...
  def apply(movementDetails: GetMovementResponse)(implicit userAnswers: UserAnswers): SubmitterType =
    if (movementDetails.consignorTrader.traderExciseNumber.contains(userAnswers.ern)) Consignor else Consignee
}
