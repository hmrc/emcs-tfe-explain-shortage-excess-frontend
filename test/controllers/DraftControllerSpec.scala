...
class DraftControllerSpec extends SpecBase with MockUserAnswersService {
      ...
      "when existing UserAnswers do not exist" - {
        ...
        "must initialise answers and redirect to when receive shortage or excess page" in new Fixture() {
          running(application) {
            ...
            val request = FakeRequest(GET, routes.DraftController.onPageLoad(testErn, testArc).url)
      ...
      "when existing UserAnswers exist" - {
        ...
        "must render the Continue Draft view" in new Fixture(Some(emptyUserAnswers.set(WhenReceiveShortageExcessPage, testDateOfWhenReceiveShortageOrExcess))) {
          ...
          running(application) {
            ...
            contentAsString(result) mustBe view(form, routes.DraftController.onSubmit(testErn, testArc)).toString()
          }
        }
      }
        ...
        "must render the page with Errors" in
          new Fixture(Some(emptyUserAnswers.set(WhenReceiveShortageExcessPage, testDateOfWhenReceiveShortageOrExcess))) {
            ...
            running(application) {
              ...
              val request = FakeRequest(POST, routes.DraftController.onPageLoad(testErn, testArc).url)
              ...
              contentAsString(result) mustBe view(boundForm, routes.DraftController.onSubmit(testErn, testArc)).toString()
            }
          }
      }
