...
trait MockGetPackagingTypesService extends MockFactory {
  ...
  object MockGetPackagingTypesService {
    ...
    def getPackagingTypes(items: Seq[MovementItem]): CallHandler2[Seq[MovementItem], HeaderCarrier, Future[Seq[MovementItem]]] =
      (mockGetPackagingTypesService.getPackagingTypes(_: Seq[MovementItem])(_: HeaderCarrier))
        .expects(where {
